...
- **환경변수** (재사용 시 폴더만 바꾸면 됨):
  ...
  - `RCLONE_BIN`, `RCLONE_REMOTE`, `GB_DRIVE_DEST` (기본값 있음)
...
### Step 2. 그림 Drive 업로드 — `scripts/upload_guidebook_images.py`
```
PYTHONUTF8=1 METACODE_ROOT="<루트>" python scripts/upload_guidebook_images.py <주차폴더>
```
- `_guidebook/images/` 의 PNG 전체를 `gdrive:메타코드/가이드북이미지/<주차>` 로 copy + 공개 권한.
- `<주차>/_guidebook/gb_drive_urls.json` 생성 (`{key: 임베드URL}`, key=파일명 stem = `output_images`의 키와 동일).
...
### Step 5. 노션 업로드 (MCP)
1. `mcp__notion__API-post-page` 로 대상 DB에 빈 페이지 생성 (제목: `📘 [가이드북] …`).
2. `mcp__notion__API-patch-block-children` 로 청크 c01부터 **순차** append.
3. 대량이면 노트북별로 업로드 에이전트에 위임 — **아래 불변 규칙 준수**.
...
3. Step2 그림 업로드 → `gb_drive_urls.json` key가 `output_images` key와 맞는지 확인.
4. Step3에서 위 "가이드북 작성 규칙"으로 spec 작성 (어투·구성 동일, output/image_ref 포함).
5. Step4 변환 → Step5 업로드(불변 규칙 준수).
6. 업로드 후 code/image 블록 보존 여부를 텍스트로 검증.
