...
### Mode B+ — 수식·예시 숫자 트레이스·시각화 + 폴더구조 발행

- **📐 수식·🔢 예시·📊 그림**: math 함수에 (a) `formula_latex`(Notion equation, KaTeX) (b) 예시 데이터를 **실제 코드로 실행해** 뽑은 값·그림을 붙인다(날조 0).
  - `scripts/trace_harness.py <패키지루트> <out>` — src를 CPU로 import·실행, 작은 예시를 파이프라인에 관통시켜 중간값(`trace.json`)+matplotlib 그림(`figures/`) 생성. **프로젝트별로 새로 작성**(NMFC판이 예시).
  - `scripts/upload_figures.py <trace.json> <figdir> <drive_urls.json>` — rclone로 Drive 업로드→공개 임베드 URL. (Notion 외부 이미지 임베드)
  - spec에 `formula_latex`·`example_note` 추가(LLM). 값·그림은 publisher가 `--trace`/`--drive-urls`로 trace에서 삽입.
- **폴더구조 + DB 발행**: `publish_code_pages.py --parent-db <database_id> --trace trace.json --drive-urls drive_urls.json` →
  인라인 DB 안에 📖개요·용어집 · 📁src(→ 패키지 폴더 → 모듈 페이지 계층) · 🔢예시 관통 페이지를 행으로 생성. 새 Notion API(data_source)에서 DB 행 생성·조회 지원. 다이어그램은 별도 DB 행에 `publish_notion.py`로.
...
- **[Mode B]** `scripts/publish_code_pages.py` — 개요/용어집 + 모듈 코드페이지 발행 + 함수 앵커 수집(멱등: 아카이브 후 재생성). `--parent-db`로 인라인 DB에 폴더구조 발행, `--trace`/`--drive-urls`로 수식·예시·그림 삽입, 파이프라인 관통 페이지 생성.
- **[Mode B+]** `scripts/trace_harness.py` — src를 실제 실행해 예시 숫자 트레이스(trace.json)+그림(figures/). **프로젝트별 작성**(NMFC판 예시).
- **[Mode B+]** `scripts/upload_figures.py` — 그림을 rclone로 Drive 업로드→Notion 임베드 URL(drive_urls.json).
